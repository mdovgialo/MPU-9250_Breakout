...
void loop()
{
  ...
  //MadgwickQuaternionUpdate(-myIMU.ay, -myIMU.ax, myIMU.az, myIMU.gx*DEG_TO_RAD,
...
myIMU.yaw   = -atan2(2.0f * (getQ()[0] * getQ()[3] + getQ()[1]*getQ()[2]),
                    1.0f-2.0f*(getQ()[2]*getQ()[2] + getQ()[3]*getQ()[3])
                    );
      myIMU.pitch = asin(2.0f * (-getQ()[0]* getQ()[2] + getQ()[3]*getQ()[1]));
      
      myIMU.roll  = -atan2( 2.0f * (getQ()[0]* getQ()[1] + getQ()[2]*getQ()[3]),
                          1.0f - 2.0f*(getQ()[1]*getQ()[1] + getQ()[2]*getQ()[2])
   ...
   hatire.gyro[0] = myIMU.yaw;
    hatire.gyro[1] =  myIMU.roll;
    hatire.gyro[2] = myIMU.pitch;
if (HATIRESEND) {
  ...
  if (!AHRS)
  {
    ...
    if (myIMU.delt_t > 50)
    {
      if(SerialDebug)
      {
        ...
        Serial.print("\"T\" : ");  Serial.print(myIMU.temperature, 1);
        Serial.println(" }");
      }
